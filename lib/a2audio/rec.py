...
class Rec:
    ...
    def getAudioFromUri(self): #function that copies file from amazon bucket to local machine
        ...
        f = None
        if self.logs:
            self.logs.write('https://' + self.bucket + '.s3.' + self.region +
                            '.amazonaws.com/' + self.uri + ' to ' +
                            self.localFiles + self.filename + self.seed)
        try:
            f = urlopen('https://' + self.bucket + '.s3.' + self.region +
                        '.amazonaws.com/' + self.uri)
            if self.logs:
