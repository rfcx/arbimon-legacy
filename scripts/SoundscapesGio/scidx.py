...
def write_scidx(filename, index, recordings, offsetx, width, offsety, height):
    ...
    fout.write(struct.pack(">6sHHHHH", "SCIDX ", VERSION, offsetx, width, offsety, height))
    ...
    # row structure :
    #     field 	desc
    #     cells_ptr[] 	array of 8 byte offsets from the beginning of the row structure - indicates begining of each entry of cells[]
    ...
    for y in range(height):
        ...
        if off_y in index:
            ...
            cell_pointers = [file_pointer_write_loc(fout, row_pointers[y].pos) for x in range(width)]
            # write each cell
            # cell structure :
            #     field 	desc
            #     count 	2 byte uint - number of recordings in indices[]
            #     indices[] 	array of rec_bytes byte uints of size count - indicates indices in recs[] lists
            for x in range(width):
                off_x = offsetx + x
                # if the row is in the index, then:::
                if off_x in row:
                    # update the cell_pointers[x] location to the current file position, relative to row_pointers[y]
                    cell_pointers[x].update()
                    cell = row[off_x]
                    fout.write(struct.pack(">H", len(cell)))
                    for rec in cell:
                        fout.write(struct.pack(rcfmt, *uint2BEbytes(rec_idx[rec], rcbytes)))

def read_scidx(filename):
    ...
    #     rows_ptr[] 	array of 8 byte file offset from start - indicates begining of each entry of rows[]
    #     rows[] 	array of row structures of size height
    magic, VERSION, offsetx, width, offsety, height = struct.unpack(">6sHHHHH", fin.read(12))
    ...
    for y in range(height):
        if row_pointers[y]:
            ...
            cell_pointers = [struct.unpack(">Q", fin.read(8)) for y in range(width)]
            # read each cell
            # cell structure :
            #     field 	desc
            #     count 	2 byte uint - number of recordings in indices[]
            ...
            for x in range(width):
                if cell_pointers[x]:
                    # seek to cell location denoted by cell_pointers[x], relative to row_pointers[y]
                    fin.seek(cell_pointers[x] + row_pointers[y])
                    cell_count, = struct.unpack(">H", fin.read(2))
                    row[offsetx + x] = [
                        recordings[BEbytes2uint(struct.pack(rcfmt, fin.read(rcbytes)))]
