...
if isinstance(rec_file, tempfilecache.CacheMiss):
    print "Fetching {} from bucket".format(rec_uri)
...
x = numpy.flipud(255*(1-((Pxx - smin)/(smax-smin))))
...
j = 0
for i in range(0,int(num_of_cols-last_tile_cols),cols_per_tile):
    png.from_array(x[:,i:(i+cols_per_tile)], 'L;8').save('/home/rafa/tile_'+str(j)+".png")
    j = j + 1

png.from_array(x[:,(num_of_cols-last_tile_cols-1):(num_of_cols-1)], 'L;8').save('/home/rafa/tile_'+str(j)+".png")
