...
async function loadProjectTieringUsage(connection, projectId) {
    const usageRows = await dbpool.queryWithConn(connection,
        'SELECT \n' +
        '    COALESCE((\n' +
        '        SELECT SUM(r.duration) / 60\n' +
        '        FROM sites s\n' +
        '        JOIN recordings r ON r.site_id = s.site_id\n' +
    ...
    const jobRows = await dbpool.queryWithConn(connection,
        ...
        '    COALESCE((SELECT COUNT(*) FROM jobs j WHERE j.project_id = ? AND j.job_type_id IN (2, 4, 5, 6, 8, 9, 10)), 0) AS job_count',
        [projectId, projectId]
    );
